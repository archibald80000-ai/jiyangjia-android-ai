...
正式知识库 v2.1 已完成 TASK-020A 本地隔离导入验收：41 approved、24 draft、65 个 2048 维真实向量，当前策略 80/80，draft 泄露 0。原始目录 `E:\work\积养家` 继续只读，禁止自动全量扫描或上传；后续新增或修改仍必须经过 `draft -> preview -> approve -> publish`。

TASK-020B 已把候选库绑定到本地 AI 数字人演示，并通过真实 ASR/RAG/LLM/TTS 与浏览器文本问答。当前待办不是重新整理知识，而是在 TASK-014H 解除后把同一版本安全发布到生产服务器并做一次生产验收。
