...
- `TASK-014H`: `BLOCKED`; after reported ICP completion, DNS is correct and public HTTP now reaches Nginx with the required `308`, but public HTTPS SNI still resets before Nginx and the Certbot dry-run still sees partial Tencent webblock propagation. Internally, fixed secrets, all-real Doubao/Ark Providers, v2.1 at 41/24 with 65 x 2048 vectors, 80/80 search, real text/WAV dialogue and the debug Demo APK download all pass. Evidence: `docs/evidence/TASK-014H/demo-online-closeout-20260810.md`.
