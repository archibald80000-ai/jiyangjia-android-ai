...
- `TASK-003`: `BLOCKED`; required `wav2lip.pth`, `s3fd.pth` and `wav2lip256_avatar1` assets are missing. Quark model and Windows package shares can be listed, but unauthenticated large-file URL creation returns `23018`, small signed URLs return HTTP `412` through CLI GET, Google Drive still times out, and local download candidate directories contain no existing copy. The launcher now has explicit `-PrepareAssets -AssetSourcePath` mode for after an authorized/manual download.
